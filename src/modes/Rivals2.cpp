...
bool input_persist; //for ModX angled tilts
int timer = 0; //for ModX angled tilts
...
void Rivals2::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
   ...
   bool shield_button_pressed = inputs.r || inputs.l;
    ...
    if (directions.diagonal && shield_button_pressed) {
            outputs.leftStickX = 128 + (directions.x * 92); // (0.77~, 0.77~) to prevent spot dodging when pressing diagonal on the ground while shielding
            outputs.leftStickY = 128 + (directions.y * 92);  
        }

    // MX Angled Tilts
    //(x, y), (71, 54), (~0.52, ~0.32) [coords, code_values, in-game values]
    if (input_persist && !inputs.a) { //input_persist becomes true if ModX + diagonal + A
        timer++;
    ...
    if (inputs.mod_x) {
        ...
        if(directions.vertical) {
            outputs.leftStickY = 128 + (directions.y * 54); // 54, 0.32~ in-game. will shield drop and give up/down tilts
        } 
