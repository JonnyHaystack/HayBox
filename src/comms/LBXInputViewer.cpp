...
LBXInputViewer::LBXInputViewer(InputSource **input_sources, size_t input_source_count)
    : CommunicationBackend(input_sources, input_source_count) {
    serial::init(115200);
}

LBXInputViewer::~LBXInputViewer() {
    serial::close();
}

void LBXInputViewer::SendReport() {
    ...
    _report[14] = ASCII_BIT(_inputs.mod_y);
    _report[15] = ASCII_BIT(_inputs.c_left);
    _report[16] = ASCII_BIT(_inputs.c_right);
    _report[17] = ASCII_BIT(_inputs.c_up);
    _report[18] = ASCII_BIT(_inputs.c_down);
    _report[19] = ASCII_BIT(_inputs.lightshield);
    _report[20] = ASCII_BIT(_inputs.midshield);
    _report[21] = ASCII_BIT(false);
    _report[22] = ASCII_BIT(false);
    _report[23] = ASCII_BIT(false);
    _report[24] = ASCII_BIT(true);
    _report[25] = '\n';

    serial::write(_report, 26);
}